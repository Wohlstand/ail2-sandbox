...
void sbi_to_ins(sbi_struct *sbi, ins_struct *ins)
{
   ...
   ins->mod_amp_vib = ((sbi->mod_sound_char & 0x40) != 0);
   ins->mod_pitch_vib = ((sbi->mod_sound_char & 0x80) != 0);
   ...
   ins->car_amp_vib = ((sbi->car_sound_char & 0x40) != 0);
   ins->car_pitch_vib = ((sbi->car_sound_char & 0x80) != 0);
...
void ins_to_sbi(ins_struct *ins, sbi_struct *sbi)
{
   ...
   sbi->mod_sound_char =
      ...
      (ins->mod_amp_vib ? 0x40:0x00) |
      (ins->mod_pitch_vib ? 0x80:0x00);

   sbi->car_sound_char =
      (ins->car_freq_mult & 0x0f) |
      (ins->car_env_scaling ? 0x10:0x00) |
      (ins->car_sustain_sound ? 0x20:0x00) |
      (ins->car_amp_vib ? 0x40:0x00) |
      (ins->car_pitch_vib ? 0x80:0x00);
