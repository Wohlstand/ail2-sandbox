...
AD_instrument *BNK1_to_AD(BNK1_instrument *src)
{
   ...
   dest.car_avekm =
      ...
      (src->car_amp_vib ? 0x40:0x00) |
      (src->car_pitch_vib ? 0x80:0x00);
   ...
   dest.mod_avekm =
      ...
      (src->mod_amp_vib ? 0x40:0x00) |
      (src->mod_pitch_vib ? 0x80:0x00);
...
#if OSI_ALE
         case TOK_ALE_FILE:
            if (ADT!=NULL) farfree(ADT);
            if (AFX!=NULL) farfree(AFX);
            make_str(buff,&lv,0);
            strcpy(buff2,buff);
